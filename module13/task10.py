...
def annuity_payment(summ, year, percent):
    k = (percent * (1 + percent) ** year) / ((1 + percent) ** year - 1)
    a_payment = round(k * summ, 2)
    return a_payment
def period_printout(summ, percent, a_payment, period):          #a_payment - как раз возвращенное значение той функции, что ранее описал
...
#debit = float(input('Введите сумму кредита: '))
debit = 140e6
#time_limit = int (input('На сколько лет выдан? '))
time_limit = 5
#rate = int (input('Сколько процентов годовых? '))
rate = 6/100
debit = annuity_payment(debit,3,rate)
period_printout (debit, rate, debit, 3)
print ('=' * 20)
n_2 = int (input('На сколько лет продляется договор? '))
i_2 = int (input('Увеличение ставки до: '))
i_2 = i_2 / 100
time_limit=time_limit-3
total = time_limit + n_2
period_printout (debit, rate, debit, 3)
print ('кроме основного долга дополнительно нужно выплатить: ', annuity_payment (debit, total , i_2))